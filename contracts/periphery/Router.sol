...
contract Router is IRouter {
    ...
    function _depositLiquidity(
        ...
        if(IFactory(factory).getTradingPair(tokenA, tokenB) == address(0)){
            IFactory(factory).createTradingPair(tokenA, tokenB);
        }
    ...
    function swapExactTokensForTokens(
        ...
        amounts = DEXLibrary.getAmountsOut(factory, amountIn, path);  //from each exchange along the path
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = DEXLibrary.sortTokens(input, output);
            ITradingPair pair = ITradingPair(DEXLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
